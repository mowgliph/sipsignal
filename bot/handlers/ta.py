...
async def ta_command(
    ...
    if is_callback and override_args:
        ...
        if not context.args:
            await message.reply_text(
                _("⚠️ Uso: `/ta <SYMBOL> [PAR] [TIME] [TV]`", user_id), parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    if is_callback:
        ...
        if target_source == "BINANCE" and not skip_binance_check:
            ...
            check_df = await loop.run_in_executor(
                None, get_binance_klines, full_symbol, timeframe, 50
            )
            if check_df is None or check_df.empty:
                await update.callback_query.answer(
                    "❌ No disponible en Binance Local", show_alert=True
                )
                return  # IMPORTANTE: Detenemos ejecución aquí, el mensaje anterior se mantiene intacto
    else:
        msg_wait = await message.reply_text(
            _("⏳ _Analizando {full_symbol} ({timeframe})..._", user_id).format(
                full_symbol=full_symbol, timeframe=timeframe
            ),
    ...
    if df_result is None:
        ...
        used_tv = True
        tv_data = await loop.run_in_executor(
            None, get_tradingview_analysis_enhanced, full_symbol, timeframe
        )
        ...
        if tv_data:
            ...
            if "STRONG_BUY" in rec:
                signal_emoji, signal_text = "🚀", "COMPRA FUERTE"
            elif "BUY" in rec:
                signal_emoji, signal_text = "🐂", "COMPRA"
            elif "STRONG_SELL" in rec:
                signal_emoji, signal_text = "🐻", "VENTA FUERTE"
            elif "SELL" in rec:
                signal_emoji, signal_text = "📉", "VENTA"
            ...
            err_txt = _(
                "❌ No se encontraron datos para *{s}* ni en Binance ni en TV.", user_id
            ).format(s=full_symbol)
            if is_callback:
...
async def ai_analysis_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        original_report_text = (
            query.message.caption if query.message.caption else query.message.text
        )

        if not original_report_text:
            await query.message.reply_text(
                _("❌ Error: No se pudo leer el reporte en pantalla.", user_id),
                parse_mode=ParseMode.MARKDOWN,
            )
            return
...
async def ta_switch_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await ta_command(
        update, context, override_source=target, override_args=[symbol, pair, timeframe]
    )
