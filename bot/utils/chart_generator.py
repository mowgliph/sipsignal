...
COLOR_EMA50 = "#ff6d00"
COLOR_EMA200 = "#9c27b0"
COLOR_BB_UPPER = "#00bcd4"
COLOR_BB_LOWER = "#00bcd4"
COLOR_BB_FILL = "#00bcd4"
COLOR_RSI = "#ff9800"
...
def _draw_candlestick(ax, df, dates):
    ...
    for _i, (date, o, h, low_price, c) in enumerate(
        ...
        # Wick
        ax.plot([date, date], [low_price, h], color=color, linewidth=0.8)

        # Body
        if abs(c - o) > 0.0001:
            ax.plot([date - 0.3, date + 0.3], [o, o], color=color, linewidth=0.8)
            ax.plot([date - 0.3, date + 0.3], [c, c], color=color, linewidth=0.8)
        else:
            # Doji
...
def _draw_bollinger_bands(ax, df):
    ...
    if "bb_upper" in df.columns and "bb_lower" in df.columns:
        ax.fill_between(
            df.index, df["bb_upper"], df["bb_lower"], alpha=0.1, facecolor=COLOR_BB_FILL
        )
...
def generate_ohlcv_chart(
    ...
    try:
        ...
        ax_price.set_title(title, color=COLOR_TEXT, fontsize=12, pad=10)
