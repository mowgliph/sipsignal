...
async def test_handle_drawdown_warns_at_50_percent():
    ...
    user_id = 1
    config = UserConfig(
        user_id=user_id, chat_id=123, capital_total=1000.0, max_drawdown_percent=5.0
    )
    ...
    state = DrawdownState(
        user_id=user_id, current_drawdown_percent=-2.4, current_drawdown_usdt=-24.0
    )
...
async def test_handle_drawdown_pauses_at_100_percent():
    ...
    state = DrawdownState(
        user_id=user_id, current_drawdown_percent=-4.9, current_drawdown_usdt=-49.0
    )
    drawdown_repo.get.return_value = state
