...
from trading.drawdown_manager import get_drawdown, reset_drawdown, resume_trading, is_trading_paused
...
async def capital_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ Error al obtener los datos. Intenta de nuevo."
        )
...
async def resume_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ Error al reanudar. Intenta de nuevo."
        )
...
async def resetdd_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "resetdd_confirm":
        try:
            ...
            await query.edit_message_text(
                "⚠️ Error al resetear. Intenta de nuevo."
            )
...
resetdd_callback_handler = CallbackQueryHandler(
    resetdd_callback, 
    pattern=r"^resetdd_(confirm|cancel)$"
)