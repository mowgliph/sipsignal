...
from db.user_config import (
    get_user_config, 
    create_or_update_user_config, 
    is_setup_completed
)
...
async def step_2_risk(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    msg = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━\\n\\n"
        "📊 *PASO 3 de 5*\\n\\n"
        f"Drawdown máximo configurado: *8%*\\n"
    ...
    keyboard = [
        [InlineKeyboardButton("✅ Mantener 8%", callback_data="dd_keep_8")]
    ]
...
setup_conversation_handler = ConversationHandler(
    ...
    states={
        STEP_1_CAPITAL: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, step_1_capital)
        ],
        STEP_2_RISK: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, step_2_risk)
        ],
        STEP_3_DRAWDOWN: [
        ...
        STEP_4_DIRECTION: [
            CallbackQueryHandler(step_4_direction, pattern="^dir_")
        ],
        STEP_5_TIMEFRAME: [
            CallbackQueryHandler(step_5_timeframe, pattern="^tf_")
        ],
        CONFIRM_SETUP: [
