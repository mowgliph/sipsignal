...
async def _handle_taken(update: Update, timestamp: int):
    ...
    try:
        ...
        await query.edit_message_text(
            f"⚠️ Error al procesar la señal: {str(e)[:100]}"
        )
...
async def _handle_skipped(update: Update, timestamp: int):
    ...
    try:
        ...
        await execute(
            "UPDATE signals SET status = 'NO_TOMADA', updated_at = NOW() WHERE id = $1",
            signal_id
        )
        ...
        await query.edit_message_text(
            f"⚠️ Error al procesar: {str(e)[:100]}"
        )
...
async def _handle_detail(update: Update, timestamp: int):
    ...
    try:
        ...
        signal = await fetchrow(
            "SELECT * FROM signals WHERE detected_at = $1 ORDER BY id DESC LIMIT 1",
            detected_dt
        )
...
async def process_signal_timeout():
    ...
    while True:
        try:
            ...
            timeout_threshold = datetime.now(timezone.utc) - timedelta(seconds=SIGNAL_TIMEOUT)
            ...
            if expired_signals:
                for signal in expired_signals:
                    ...
                    logger.warning(f"⏰ Señal {signal['id']} marcada como SIN_RESPUESTA por timeout")
            
            if expired_signals:
                logger.info(f"⏰ Timeout procesado: {len(expired_signals)} señales actualizadas a SIN_RESPUESTA")
...
signal_response_handler = CallbackQueryHandler(
    signal_response_callback,
    pattern=r'^(taken:|skipped:|detail:)(LONG|SHORT)_\d+$'
)
