...
async def get_or_create_drawdown(user_id: int) -> Dict[str, Any]:
    ...
    dd = await fetchrow(
        "SELECT * FROM drawdown_tracker WHERE user_id = $1",
        user_id
    )
...
async def get_drawdown(user_id: int) -> Optional[Dict[str, Any]]:
    """
...
async def update_drawdown(user_id: int, pnl_usdt: float, bot) -> Dict[str, Any]:
    ...
    current_drawdown_percent = (current_drawdown_usdt / capital_total * 100) if capital_total > 0 else 0
    ...
    logger.info(f"📉 Drawdown actualizado para user {user_id}: {current_drawdown_usdt:.2f} USDT ({current_drawdown_percent:.2f}%)")
...
async def is_trading_paused(user_id: int) -> bool:
    ...
    """
    dd = await fetchrow(
        "SELECT is_paused FROM drawdown_tracker WHERE user_id = $1",
        user_id
    )
