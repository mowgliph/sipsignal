...
class PriceMonitor:
    ...
    def __init__(self):
        ...
        self._ws: Optional[aiohttp.WebSocketRunner] = None
        self._reconnect_attempts = 0
        self._reconnect_delay = RECONNECT_DELAY
        self._current_price: Optional[float] = None
        self._last_ticker_update: Optional[datetime] = None
    ...
    async def _process_ticker(self, data: Dict[str, Any]):
        ...
        self._last_ticker_update = datetime.now(timezone.utc)
    ...
    async def _notify_tp1(self, trade: Any, entry_price: float, tp1_level: float, current_price: float):
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ TP1 tomado y SL movido", callback_data=f"pm_tp1_done:{trade_id}"),
    ...
    async def _notify_sl(self, trade: Any, entry_price: float, sl_level: float, current_price: float):
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ Cerré la posición", callback_data=f"pm_sl_closed:{trade_id}"),
                InlineKeyboardButton("📊 Ver resumen", callback_data=f"pm_sl_summary:{trade_id}")
            ]
        ])
        
        await self._send_notification(message, keyboard)
        logger.info(f"🛑 SL alcanzado para trade {trade_id} ({direction}) - Pérdida: {loss_usdt:.2f} USDT")
    ...
    async def _handle_reconnect(self):
        ...
        if self._reconnect_attempts > MAX_RECONNECT_ATTEMPTS:
            logger.error(f"❌ Máximo de intentos de reconexión alcanzado ({MAX_RECONNECT_ATTEMPTS})")
            self._running = False
            return
        
        logger.warning(f"⚠️ Reconectando en {self._reconnect_delay}s (intento {self._reconnect_attempts}/{MAX_RECONNECT_ATTEMPTS})")
